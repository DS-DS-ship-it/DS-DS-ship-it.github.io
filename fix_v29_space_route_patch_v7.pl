...
sub astro_block_range {
    my $needle = "    astro:";
    my $start = index($html, $needle);
    die "Patch failed: astro block not found\n" if $start < 0;

    my $brace = index($html, '{', $start);
    die "Patch failed: astro opening brace not found\n" if $brace < 0;
...
# 1) Add missing astro route state robustly
if (index($html, 'earthRoute2d') < 0) {
    my ($start, $brace, $end) = astro_block_range();
    my $block = substr($html, $start, $end - $start + 1);

    my $insert = "\n      earthRoute2d: null,\n      earthRoute2dMarkers: null,\n      earthRoute3d: null,";

    if ($block =~ /cursorDec\s*:\s*null\s*,?/) {
        $block =~ s/(cursorDec\s*:\s*null\s*,?)/$1$insert/;
    } elsif ($block =~ /currentCatalogFilter\s*:/) {
        $block =~ s/(currentCatalogFilter\s*:)/earthRoute2d: null,\n      earthRoute2dMarkers: null,\n      earthRoute3d: null,\n      $1/;
    } else {
        substr($block, -1, 0, ",$insert\n");
    }

    substr($html, $start, $end - $start + 1, $block);
    print "Patched: add astro route state\n";
} else {
    print "Skipped: astro route state already present\n";
}

# 2) Add helper functions if missing
...
# 6) Populate selectors at boot if missing
